...
# DPR non-transfomred data
   	m0 = lmer(log(DPR) ~ log( N_nests) + hemisphere + mean_year+lat_abs +(1|site)+(1|species),  data =  dd_)
	...
	m0p = lmer(log(DPR) ~ log( N_nests) + mean_year+poly(Latitude,3) +(1|site)+(1|species),  data =  dd_)
	m_ass(name = 'DDPR_year+Poly3LatTRUE', mo = m0p, dat = dd_, fixed = c('N_nests','mean_year', 'Latitude'),categ = NULL, trans = c('log','none','none'), spatial = TRUE, temporal = TRUE, PNG = TRUE)
	om0p = m_out(name = "DPR_year+Poly3LatTRUE'", model = m0p, round_ = 3, nsim = 5000, aic = FALSE)
	
	m1 = lmer(log(DPR) ~ log( N_nests) + hemisphere*mean_year*lat_abs +(1|site)+(1|species),  data =  dd_)
	...
	m1p = lmer(log(DPR) ~ log( N_nests) + mean_year*poly(Latitude,3) +(1|site)+(1|species),  data = dd_)
...
# TPR
   	m0 = lmer(TPR ~ log( N_nests) + hemisphere + mean_year+lat_abs +(1|site)+(1|species),  data =  dd_)
	...
	m0p = lmer(TPR ~ log( N_nests) + mean_year+poly(Latitude,3) +(1|site)+(1|species),  data =  dd_)
	...
	m1 = lmer(TPR ~ log( N_nests) + hemisphere*mean_year*lat_abs +(1|site)+(1|species),  data =  dd_)
	...
	m1p = lmer(TPR ~ log( N_nests) + mean_year*poly(Latitude,3) +(1|site)+(1|species),  data = dd_)
