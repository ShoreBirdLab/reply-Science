...
{# <2000 and only Beintema nests
if(PNG == TRUE) {
		png(paste(outdir,"sensitivity_anal_less2000_BeintemaNests.png", sep=""), width=1.85,height=1.85,units="in",res=600)	 
		#	png(paste(outdir,"Figure_1a_Arc_temp.png", sep=""), width=1.85,height=1.85,units="in",res=600)	 
					#jpeg(paste(outdir,"Figure_3.jpeg", sep=""), width=3.5,height=1.85,units="in",res=600) 
					}else{
					dev.new(width=1.85,height=1.85) #dev.new(width=3.5, height=1.97)
...
for(i in 1: length(seq(0.1,0.9, by = 0.1))){
 ...
 m = lmer(log(DPR+0.01) ~ ln_N_nests + mean_year + (1|mean_year) +(1|Latitude)+(1|species),  data = b[b$mean_year<2000 & b$DPRtrans=='YES' ,])  
 #plot(allEffects(m))
 #summary(glht(m))
 bsim <- sim(m, n.sim=nsim)  
 v = apply(bsim@fixef, 2, quantile, prob=c(0.5))
 # values to predict for		
   newD=data.frame(ln_N_nests = mean(b$ln_N_nests[b$mean_year<2000 & b$DPRtrans=='YES']),mean_year = seq(min(b$mean_year[b$mean_yea<2000 & b$DPRtrans=='YES']),max(b$mean_year[b$mean_year<2000 & b$DPRtrans=='YES']), length.out=300))
 # exactly the model which was used has to be specified here 
   X <- model.matrix(~ ln_N_nests + mean_year,data=newD)	
 # calculate predicted values and creditability intervals
   newD$pred <- X%*%v # #newD$fit_b <- plogis(X%*%v) # in case on binomial scaleback
