...
# plot
if(PNG == TRUE) {
   png(paste(outdir,"Figure_1panels8.png", sep=""), width=4*1.6,height=2*1.45,units="in",res=600) 
	}else{dev.new(width=4*1.6,height=2*1.45)
  }
...
# B period
  ...
  text(x = 80, y =0.07*0.98, labels= expression(bold("C")), col='black', cex = 0.7, xpd = TRUE)
...
# Proportion of transformed
  ...
  mtext('Population\nwith assumed exposure',side=2,line=1.1, cex=0.6, las=3, col='grey30')
  text(x = 2016, y =100*0.98, labels= expression(bold("E")), col='black', cex = 0.7, xpd = TRUE)
  #text(x = 2016, y =100, labels= expression(bold("G")), col='black', cex = 0.7, adj = 0)
	
  polygon(c(a$mean_year, rev(a$mean_year)), c(a$lwr, 
	  rev(a$upr)), border=NA, col = adjustcolor('black',alpha.f = 0.1), xpd = FALSE)#adjustcolor(col_t ,alpha.f = 0.2)) #0,0,0 black 0.5 is transparents RED
...
# Simulation
  ...
  par(mar=c(.1,2.5,1,0),ps=12, cex=1, font.main = 1, cex.lab=0.6,cex.main=0.7, cex.axis=0.5, tcl=-0.05,bty="n",xpd=FALSE)
  ...
  axis(2, at=seq(0,0.16, by = 0.02), labels=seq(0,0.16, by = 0.02), lwd = 0.5)
			mtext('Daily nest predation',side=2,line=1.1, cex=0.6, las=3, col='grey30')
  text(x = c(2005), y =0.157-1*0.06923077*0.157, labels= c('Observed'), col='grey18', cex = 0.5, adj = 1)
  text(x = c(2005), y =0.157-2*0.06923077*0.157, labels= c('nesting'), col='grey18', cex = 0.5, adj = 1)
  text(x = c(2005), y =0.157-3*0.06923077*0.157, labels= c('period'), col='grey18', cex = 0.5, adj = 1)
  ...
  text(x = 2016, y =0.16*0.98, labels= expression(bold("F")), col='black', cex = 0.7, xpd = TRUE)		
for(i in 1: length(seq(0.1,0.9, by = 0.1))){
 #i=4
...
# G
 ...
 plot(u$N_nests~u$DPR_orig, pch=19,xlim=c(0,0.2), ylim=c(0,0.8),ylab=NA,xlab=NA, xaxt='n',yaxt='n', type='n')  
  ...
  axis(2, at=seq(0,0.8, by = 0.2), labels=seq(0,0.8, by = 0.2), lwd = 0.5)
			mtext('Estimated daily nest predation',side=2,line=1.1, cex=0.6, las=3, col='grey30')
...
abline(0,1, lwd = 0.5, lty = 3, col = 'black', xpd=FALSE)
text(x = 0.2, y =0.8*0.98, labels= expression(bold("G")), col='black', cex = 0.7, xpd = TRUE)	
...
# A limited
  ...
  axis(2, at=seq(0,0.07, by = 0.01), labels=seq(0,0.07, by = 0.01), lwd = 0.5)
	mtext('Daily nest predation',side=2,line=1.1, cex=0.6, las=3, col = 'black')
  text(x = 2016, y =0.07*0.98, labels= expression(bold("H")), col='black', cex = 0.7,  xpd = TRUE)
  for(i in unique(pp$Belt)){
		#i ="Arctic"
		print(i)
		ppi = pp[pp$Belt==i,]
